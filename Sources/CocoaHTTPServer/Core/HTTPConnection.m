...
- (void)replyToHTTPRequest
{
	...
	if (HTTP_LOG_VERBOSE)
	{
		...
		HTTPLogVerbose("Received HTTP request:\n%@", tempStr);
	}
	...
	if ([WebSocket isWebSocketRequest:request])
	{
		...
		else
		{
			...
			{
				HTTPLogWarn("WebSocket forgot to set itself as socket delegate");
...
- (NSString *)filePathForURI:(NSString *)path allowDirectory:(BOOL)allowDirectory
{
	...
	if (documentRoot == nil)
	{
		HTTPLogWarn("No configured document root");
		return nil;
	}
	...
	if (docRoot == nil)
	{
		HTTPLogWarn("Document root is invalid file path");
		return nil;
	}
	...
	{
		HTTPLogWarn("Request for file outside document root");
		return nil;
	}
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData*)data withTag:(long)tag
{
		...
		else
		{
			...
			if (expectsUpload)
			{
				...
				else
				{
					...
					{
						HTTPLogWarn("Method expects request body, but had no specified Content-Length");
					...
					{
						HTTPLogWarn("Unable to parse Content-Length header into a valid number");
			...
			else
			{
				if (contentLength != nil)
				{
					...
					{
						HTTPLogWarn("Method not expecting request body had non-zero Content-Length");
	...
	else
	{
		...
		if (tag == HTTP_REQUEST_CHUNK_SIZE)
		{
			...
			if (errno != 0)
			{
				HTTPLogWarn("Method expects chunk size, but received something else");
		...
		else if (tag == HTTP_REQUEST_CHUNK_TRAILER)
		{
			...
			{
				HTTPLogWarn("Method expects chunk trailer, but is missing");
...
- (void)responseHasAvailableData:(NSObject<HTTPResponse> *)sender
{
	...
	dispatch_async(connectionQueue, ^{ @autoreleasepool {
		...
		{
			HTTPLogWarn("Sender is not current httpResponse");
			return;
		}
