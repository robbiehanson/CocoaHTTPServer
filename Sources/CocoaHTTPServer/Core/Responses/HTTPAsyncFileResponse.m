...
- (void)pauseReadSource
{
	...
	{
		HTTPLogVerbose("Suspending readSource");
...
- (void)resumeReadSource
{
	if (readSourceSuspended)
	{
		HTTPLogVerbose("Resuming readSource");
...
{
	HTTPLogVerbose("Canceling readSource");
...
- (BOOL)openFileAndSetupReadSource
{
	...
	if (fileFD == NULL_FD)
	{
		HTTPLogError("Unable to open file. filePath: %@", filePath);
		
		return NO;
	}
	
	HTTPLogVerbose("Open fd[%i] -> %@", fileFD, filePath);
	...
	dispatch_source_set_event_handler(readSource, ^{
		...
		if (readBuffer == NULL || bytesToRead > (readBufferSize - readBufferOffset))
		{
			...
			if (readBuffer == NULL)
			{
				HTTPLogError("Unable to allocate buffer");
		...
		HTTPLogVerbose("Attempting to read %lu bytes from file", (unsigned long)bytesToRead);
		...
		{
			HTTPLogVerbose("Read %lu bytes from file", (unsigned long)result);
	...
	dispatch_source_set_cancel_handler(readSource, ^{
		...
		HTTPLogTrace2("cancelBlock - Close fd[%i]", theFileFD);
...
{
	HTTPLogTrace2("contentLength - %llu", fileLength);
...
{
	HTTPLogTrace2("setOffset:%llu", offset);
	...
	if (result == -1)
	{
		HTTPLogError("lseek failed - errno(%i) filePath(%@)", errno, filePath);
...
{
	HTTPLogTrace2("readDataOfLength:%lu", (unsigned long)length);
	
	if (data)
	{
		NSUInteger dataLength = [data length];
		
		HTTPLogVerbose("Returning data of length %lu", (unsigned long)dataLength);
...
- (BOOL)isDone
{
	...
	HTTPLogTrace2("isDone - %@", (result ? @"YES" : @"NO"));
