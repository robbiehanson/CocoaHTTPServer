...
- (id) initWithBoundary:(NSString*) boundary formEncoding:(NSStringEncoding) _formEncoding {
	...
	if( nil == boundary ) {
		HTTPLogWarn(@"MultipartFormDataParser: init with zero boundary", nil);
		return nil;
	}
	...
	while( true ) {
		...
		if( contentEnd == -1 ) {
			...
			if( [delegate respondsToSelector:@selector(processContent:WithHeader:)] ) {
				HTTPLogVerbose(@"MultipartFormDataParser: Processed %ld bytes of body",(unsigned long)sizeToPass);
