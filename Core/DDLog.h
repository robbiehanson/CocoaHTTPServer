...
/*
 * Kludge.  Some genius decided to deprecate a function we use a lot (dispatch_get_current_queue()).  Until the upstream
 * project is revised to compile cleanly, reduce the flood of warnings to just one.  This is the most commonly included
 * header file so do it here to keep future merges as simple as possible.
 */
#define dispatch_get_current_queue get_current_dispatch_queue
extern dispatch_queue_t get_current_dispatch_queue(void);
